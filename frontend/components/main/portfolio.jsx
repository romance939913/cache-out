...
function Portfolio(props) {    
  ...
  props.tickers.forEach((ticker, idx) => {
    if (props.holdings[ticker].quantity !== 0) {
      let data = props.graphPrices[ticker]
      let dayDifference;
      let percentage;
      let color;
      let renderLineChart
      ...
      if (day === 6 || day === 0 || hd.type === 'public' || hd.type === 'bank') {
        data = data.filter(obj => {
          let lastTradingDay = moment(props.graphPrices[ticker][0].date.split(" ")[0])
      ...
      tickerArr.push(<Link
        ...
        <li key={idx} className="holding-portfolio-li">
          <div className="portfolio-ele portfolio-ele-left">
            <h1>{ticker}</h1>
            <p className="portfolio-ele-small">{numeral(props.holdings[ticker].quantity).format('0,0')} shares</p>
          </div>
          <div className="chart-thumbnail">
            {renderLineChart}
          </div>
          <div className="portfolio-ele portfolio-ele-right">
            <p>{numeral(props.price[ticker]).format('$0,0.00')}</p>
